    ...
    public class Funciones_Devolucion
    {
        ...
        public bool EliminarDevolucion(int idDevolucion)
        {
            ...
            try
            {
                ...
                {
                    using (OracleCommand command = new OracleCommand("ELIMINARDEVOLUCION", connection))
                    {
        ...
        public bool RegistrarDevolucion(int idDetalle, int cantidad, string motivo)
        {
            bool exito = false;
            ...
            try
            {
                using (OracleConnection connection = Conn.GetOpenConnection())
                {
                    using (OracleCommand command = new OracleCommand("REGISTRARDEVOLUCION", connection))
                    {
                        command.CommandType = System.Data.CommandType.StoredProcedure;
                        // Parámetros de entrada
                        command.Parameters.Add("p_id_detalle", OracleDbType.Int32).Value = idDetalle;
                        command.Parameters.Add("p_fecha_devolucion", OracleDbType.Date).Value = DateTime.Now;
        ...
        public bool ModificarDevolucion(int idDevolucion, int idDetalle, int cantidad, string motivo)
        {
            bool exito = false;
            ...
            try
            {
                using (OracleConnection connection = Conn.GetOpenConnection())
                {
                    using (OracleCommand command = new OracleCommand("MODIFICARDEVOLUCION", connection))
                    {
                        command.CommandType = System.Data.CommandType.StoredProcedure;
        ...
        public DataSet ObtenerDevoluciones()
        {
            ...
            try
            {
                using (OracleConnection connection = Conn.GetOpenConnection())
                {
                    string sqlQuery = "SELECT * FROM VISTADEVOLUCIONES";
